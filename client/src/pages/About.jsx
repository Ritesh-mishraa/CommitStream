...
const About = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-950 pt-32 pb-20 px-6 font-sans overflow-x-hidden">
            <div className="max-w-4xl mx-auto">
                ...
                >
                    <div className="bg-slate-900/30 p-8 rounded-2xl border border-slate-800">
                        ...
                        <h3 className="text-xl font-semibold text-slate-200 mb-2">Precision</h3>
                        <p className="text-sm text-slate-400">Accurately predicting conflicts with deep code intelligence saves hours of manual resolution.</p>
                    </div>
                    <div className="bg-slate-900/30 p-8 rounded-2xl border border-slate-800">
                        <Zap className="w-8 h-8 text-emerald-400 mb-4" />
                        <h3 className="text-xl font-semibold text-slate-200 mb-2">Speed</h3>
                        <p className="text-sm text-slate-400">Real-time collaborative editing means issues are addressed instantly by the right stakeholders.</p>
                    </div>
                    <div className="bg-slate-900/30 p-8 rounded-2xl border border-slate-800">
                        <Shield className="w-8 h-8 text-purple-400 mb-4" />
                        <h3 className="text-xl font-semibold text-slate-200 mb-2">Reliability</h3>
                        <p className="text-sm text-slate-400">Keeping your main branch pristine and ensuring every merge is secure and fully vetted.</p>
                    </div>
                </motion.div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 max-w-3xl mx-auto">
                        {team.map((member, idx) => (
                            <div key={idx} className="flex flex-col items-center group">
                                <div className="w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center text-2xl font-bold text-slate-300 mb-6 border-2 border-slate-700 group-hover:border-blue-500 transition-colors shadow-xl">
                                    {member.avatar}
                                </div>
                                <h3 className="text-xl font-semibold text-slate-200">{member.name}</h3>
                                <p className="text-sm text-blue-400 mb-4">{member.role}</p>
                                <p className="text-sm text-slate-400 text-center mb-6 leading-relaxed max-w-xs">{member.bio}</p>
