...
const JoinProject = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4 selection:bg-blue-500/30">
            <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl shadow-xl overflow-hidden text-center p-8 space-y-6">
                ...
                {status === 'loading' && (
                    <div className="flex flex-col items-center">
                        ...
                        <h2 className="text-xl font-bold text-white mb-2">Joining Workspace...</h2>
                        <p className="text-slate-400 text-sm">Validating your invite link securely.</p>
                    </div>
                )}
                ...
                {status === 'success' && project && (
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-slate-400 text-sm mb-8">
                            You are now a member of the <strong className="text-slate-200">{project.name}</strong> workspace.
                        </p>
                        <button
                            onClick={() => navigate('/dashboard')}
                            className="bg-blue-600 hover:bg-blue-500 text-white w-full py-2.5 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors"
                        >
                ...
                {status === 'error' && (
                    <div className="flex flex-col items-center">
                        ...
                        <p className="text-slate-400 text-sm mb-8 bg-black/20 p-3 rounded-lg border border-red-500/10 inline-block">
                            {errorMessage}
                        </p>
                        <button
                            onClick={() => navigate('/dashboard')}
                            className="bg-slate-800 hover:bg-slate-700 text-white w-full py-2.5 rounded-lg text-sm font-medium transition-colors"
                        >
                            Return to Dashboard
                        </button>
                    </div>
