...
const Layout = () => {
    ...
    return (
        <div className="flex h-screen w-full bg-slate-950 text-slate-400 overflow-hidden font-sans">

            {/* Sidebar */}
            <aside className="w-64 border-r border-slate-800 bg-slate-900/50 flex flex-col">
                <div className="p-6">
                    <div className="flex items-center gap-2 mb-8 text-slate-100 font-medium">
                ...
                ${isActive ? 'bg-blue-500/10 text-blue-400' : 'hover:bg-slate-800/50 hover:text-slate-200'}
              `}
                        >
            ...
            <main className="flex-1 flex flex-col relative overflow-hidden">
                {/* Topbar */}
                <header className="h-14 border-b border-slate-800 flex items-center px-6 justify-between bg-slate-900/20 backdrop-blur-sm z-10 hidden sm:flex">
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-slate-500 text-xs font-mono">WORKSPACE</span>
                        <span className="text-slate-600">/</span>
                        <span className="text-slate-200 font-medium capitalize">CommitStream MVP</span>
                    </div>

                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2 px-3 py-1 bg-slate-900 border border-slate-800 rounded-full">
                            <Activity className="w-3 h-3 text-emerald-500 animate-pulse" />
                            <span className="text-xs text-slate-400 font-mono">SYS_OK</span>
                        </div>
                    </div>
                </header>
