                ...
                <div className="flex items-center justify-between p-5 border-b border-slate-800">
                    <div>
                        <h2 className="text-lg font-semibold text-slate-100 flex items-center gap-2">
                            ...
                            History for <code className="text-sm bg-slate-950 px-2 py-0.5 rounded text-blue-300">{branchName}</code>
                        </h2>
                        <p className="text-xs text-slate-400 mt-1">Recent modifications and authors</p>
                    </div>
                        ...
                        <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-800 before:to-transparent">
                            {commits.map((commit, idx) => (
                                <div key={commit.sha} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                                    {/* Icon */}
                                    <div className="flex items-center justify-center w-6 h-6 rounded-full border border-slate-800 bg-slate-900 text-slate-500 shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10 shadow-sm">
                                        <div className="w-1.5 h-1.5 bg-blue-500 rounded-full"></div>
                                    </div>
                                    {/* Content */}
                                    <div className="w-[calc(100%-2.5rem)] md:w-[calc(50%-1.5rem)] p-4 rounded border border-slate-800 bg-slate-900/50 shadow-sm">
                                        <div className="flex items-center justify-between mb-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                <span className="text-xs font-medium text-slate-300">{commit.authorName}</span>
                                            </div>
                                        ...
                                        <p className="text-sm text-slate-200 mb-2 whitespace-pre-wrap">{commit.message}</p>
                                        ...
                                        {commit.files && commit.files.length > 0 && (
                                            <div className="mt-3 pt-3 border-t border-slate-800 space-y-1">
                                                ...
                                                {commit.files.map(f => (
                                                    <div key={f.filename} className="text-xs flex items-center gap-2 text-slate-300 bg-slate-950 pb-1 pt-1 px-2 rounded font-mono truncate">
